...
namespace XMLTranlsatorHelper
{
    public partial class Main : Form
    {
        ...
        private void AddListViewItem(XmlNode node)
        {
            ...
            {
                if (node.NodeType != XmlNodeType.Comment && node.NodeType != XmlNodeType.CDATA && node.NodeType != XmlNodeType.SignificantWhitespace)
                {
        ...
        private void Form1KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (e.KeyCode == Keys.F3 && !e.Control && !e.Alt)
            {
                ...
                e.Handled = true;
                e.SuppressKeyPress = true;
            }else if (e.KeyCode == Keys.Enter && IsKeyLocked(Keys.Scroll))
            {
